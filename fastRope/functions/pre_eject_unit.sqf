...
if (_forceDis == 0) then
{
	[_unit,_cargoPos,_forceDis] spawn NORRN_FR_EU;
} else {
	if (isplayer _unit) then 
	{
		_unit setVariable ["NORRN_FR_pD", false, true]; //global variable - player agrees to descend fastrope 
		NORRN_descend_rope_act = _heli addAction ["Descend fast rope", "fastRope\link_unit.sqf", [_unit,_cargoPos,_forceDis], 10, true, true, ""];
		...
		if (Nor_FR_AIcomplete && !(_unit getVariable "NORRN_FR_pD") && !(_heli getVariable "NORRN_fastRopeComplete")) then {[_unit,_cargoPos,_forceDis] spawn NORRN_FR_EU};
