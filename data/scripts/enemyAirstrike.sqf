...
BIS_EVO_SetVelocityVector =
{
    ...
    //	systemChat format ["bomb vel: %1, dir: %2, pos: %3",_speed,_direction,position _object];
};
    _range = 500;	
    ...
    _heli0 = createVehicle ["FRL_AV8B_MR",_pos1, [], 100, "FLY"];
    ...
    if (canmove _heli0) then 
    {
        ...
        while {_i <= _count} do 
        {
            ...
            _k = createVehicle ["EB_Bo_FAB500",[(position _heli0 select 0), (position _heli0 select 1), (position _heli0 select 2)-1], [], 0, "NONE"];
            [_k, getDir _heli0, 100] call BIS_EVO_SetVelocityVector;
