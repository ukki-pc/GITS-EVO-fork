...
BIS_EVO_ListUpdate = {
    ...
    */
    ...
    TerList = [
        [localize "str_terrain_50", 50, 0],
        [localize "str_terrain_25", 25, 1],
        [localize "str_terrain_12_5", 12.5, 2],
        [localize "str_terrain_6_25", 6.25, 3],
        [localize "str_terrain_3_125", 3.125, 4]
    ];
    ...
    */
    ...
    //support
    SupList = SupList + [
        [localize "str_dn_ammocrate", "Cost: 5 All the ammo you need", 10, 5, 10, "data\sup01.paa", 1]
    ];
    SupList = SupList + [
        ["Vehicle ammo crate", "Cost: 10 All the Vehicle ammo you need", 10, 10, 10, "data\sup01.paa", 25]
    ];
    SupList = SupList + [
        ["Fuel barrels", "Cost: 10 All the fuel you need", 10, 10, 10, "data\sup01.paa", 26]
    ];
    //	SupList = SupList +[["Player Air Support","Control the plane you use",10,10,10,"data\sup01.paa",27]];
    SupList = SupList + [
        [localize "strwfsmokegreen", "Cost: 0 Mark a location with white/green smoke", 10, 0, 3, "data\sup01.paa", 9]
    ];
    SupList = SupList + [
        [localize "str_dn_bike", "Emergency transport", 20, 0, 10, "data\sup01.paa", 19]
    ];
    SupList = SupList + [
        [localize "str_som_marker_boat", "Cost: 0 Ideal for crossing water", 20, 0, 10, "data\sup01.paa", 10]
    ];
    SupList = SupList + [
        [localize "str_som_arty_barrage_mission_4", "Cost: 3 Ideal for night defence", 20, 3, 20, "data\sup01.paa", 20]
    ];
    //precision strike
    SupList = SupList + [
        [localize "STR_M04t106", localize "STR_M04t107", 20, (EGG_EVO_Artycost * 2), 10, "data\sup01.paa", 2]
    ];
    //blazes change
    //new mortar barrage
    SupList = SupList + [
    ...
    AssList = [];
    AssList = AssList + [
        ["Relocate", "Relocate to captured town or base.", "data\offensive.paa", 0, 17]
    ];
    AssList = AssList + [
        ["Town Conquest", "Capture objective from enemy control.", "data\offensive.paa", 0, 16]
    ];
    AssList = AssList + [
        ["Change Spawnpoint", "Pick a friendly location to spawn to.", "data\offensive.paa", 0, 19]
    ];
    AssList = AssList + [
    ...
    if (_sco >= BIS_EVO_rank1) then {
        ...
        SupList = SupList + [
            [localize "str_dn_motorcycle", "Cost: 5 Emergency transport for two", 20, 5, 10, "data\sup01.paa", 8]
        ];
        //place mines
        SupList = SupList + [
            [localize "str_paradrop_obj12shorter", "Ideal for protecting a road", 50, (EGG_EVO_Artycost * 2), 5, "data\sup01.paa", 22]
        ];

        //place traps
        if (gitsnades == 2) then {
            SupList = SupList + [
                ["Set traps", "Ideal for protecting an area against infantry", 50, (EGG_EVO_Artycost * 2), 5, "data\sup01.paa", 24]
            ]
        };
    };
    if (_sco >= BIS_EVO_rank2) then {
        ...
        //small arty
        SupList = SupList + [
            [localize "STR_M04t108", localize "STR_M04t109", 50, (EGG_EVO_Artycost * 4), 20, "data\sup01.paa", 3]
        ];
    ...
    if (_sco >= BIS_EVO_rank3) then {
        ...
        SupList = SupList + [
            [localize "str_dn_fortification", "Cost: 10 Ideal for securing an area and providing cover", 20, 10, 10, "data\sup01.paa", 14]
        ];
        //cluster bomb
        SupList = SupList + [
            [localize "STR_M04t110", localize "STR_M04t111", 50, (EGG_EVO_Artycost * 6), 20, "data\sup01.paa", 4]
        ];
    };
    if (_sco >= BIS_EVO_rank4) then {
        ...
        //support	
        SupList = SupList + [
            [localize "str_dn_rhib2", "Cost: 5 Ideal for assaulting positions from the sea", 20, 5, 10, "data\sup01.paa", 15]
        ];
        SupList = SupList + [
            [localize "init.sqf7_3", "Cost: 20 Ideal for Air Defences", 20, 20, 10, "data\sup01.paa", 16]
        ];
        SupList = SupList + [
            [localize "str_artdlg_btnfirearea", "Ideal for suppressing a city", 200, (EGG_EVO_Artycost * 5), 30, "data\sup01.paa", 21]
        ];
        //large arty
        SupList = SupList + [
            [localize "STR_M04t112", localize "STR_M04t113", 100, (EGG_EVO_Artycost * 8), 30, "data\sup01.paa", 5]
        ];
    };
    if (_sco >= BIS_EVO_rank5) then {
        ...
        if (helpersparam == 1) then {
            RecList = RecList + [
                [getText(configFile >> "CfgVehicles" >> "US_Delta_Force_SD_EP1" >> "DisplayName"), "US_Delta_Force_SD_EP1", "\Ca\characters\data\Ico\i_sf_ca.paa", ["US_Delta_Force_SD_EP1"] call _GetGuns]
            ];
        };

        //support
        SupList = SupList + [
            [localize "str_dn_staticcannon", "Cost: 25 Ideal for suppressing enemy armour and troops", 20, 25, 10, "data\sup01.paa", 17]
        ];
        //chem strike
        SupList = SupList + [
            [localize "STR_M04t114", localize "STR_M04t115", 150, (EGG_EVO_Artycost * 20), 40, "data\sup01.paa", 6]
        ];
    ...
    if (_sco >= BIS_EVO_rank6) then {
        ...
        //rank 6
        SupList = SupList + [
            [localize "init.sqf_5_1", "Cost: 5 Get me out of this hell-hole", 20, 5, 5, "data\sup01.paa", 18]
        ];
        //nuke
        SupList = SupList + [
            [localize "STR_M04t116", localize "STR_M04t117", 200, (EGG_EVO_Artycost * 40), 60, "data\sup01.paa", 7]
        ];
    };
    ...
    */
    ...
    if (helpersparam == 2) then {
        ...
        forEach BIS_EVO_recruitables;
        RecList = RecList + [
            ["Copy me", "ME", "\Ca\characters\data\Ico\i_med_ca.paa", [player] call _GetMyGuns]
        ];
    };
...
BIS_EVO_ActButton = {
    ...
    if (_recpage) then {
        if (helpersparam == 0) exitwith {
            ctrlSetText[2011, "AI Recruitment disabled"]
        };
        ...
        if (player hasWeapon "ItemRadio") then {
            ...
            if (not BIS_EVO_fmission) then {
                if (bplace) then {
                    ...
                    ctrlSetText[2011, localize "STR_M04t90"]
                }; //Click a point on the map first.
            } else {
                ctrlSetText[2011, localize "STR_M04t91"]
            }; //Only one payload can be delivered at a time.
            //EGG editing
        } else {
            ctrlSetText[2011, "You must have a radio to call support"]
        };
    };
        ...
        if (perkPoints > 0) then {
            switch (_index)
                do {
                    ...
                    case 1: {
                        if (perkEngLVL < 4) then {
                            perkEngLVL = perkEngLVL + 1;
                        }
                    };
                    case 2: {
                        if (perkSniperLVL < 4) then {
                            perkSniperLVL = perkSniperLVL + 1;
                        }
                    };
                    case 3: {
