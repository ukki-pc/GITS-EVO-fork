...
BIS_EVO_Rank = 
{
	...
	if (_score < BIS_EVO_rank2 and _score >= BIS_EVO_rank1 and _rank != "CORPORAL")  exitWith  
	{
		_rname = format["Corp.%1",_name];
		_hint =  format[_rankmes,_rname];
		["corp","CORPORAL",_hint] call BIS_EVO_Rdisp;
	...
	if (_score < BIS_EVO_rank3 and _score >= BIS_EVO_rank2 and _rank != "SERGEANT")  exitWith  
	{
		_rname = format["Sgt.%1",_name];
		_hint =  format[_rankmes,_rname];
		["sgt","SERGEANT",_hint] call BIS_EVO_Rdisp;		
	...
	if (_score < BIS_EVO_rank4 and _score >= BIS_EVO_rank3 and _rank != "LIEUTENANT")  exitWith  
	{	
		_rname = format["Ltn.%1",_name];
		_hint =  format[_rankmes,_rname];
		["ltn","LIEUTENANT",_hint] call BIS_EVO_Rdisp;		
	...
	if (_score < BIS_EVO_rank5 and _score >= BIS_EVO_rank4 and _rank != "CAPTAIN")  exitWith  
	{
		_rname = format["Cpt.%1",_name];
		_hint =  format[_rankmes,_rname];
		["cpt","CAPTAIN",_hint] call BIS_EVO_Rdisp;		
	...
	if (_score < BIS_EVO_rank6 and _score >= BIS_EVO_rank5 and _rank != "MAJOR")  exitWith  
	{	
		_rname = format["Mjr.%1",_name];
		_hint =  format[_rankmes,_rname];
		["mjr","MAJOR",_hint] call BIS_EVO_Rdisp;		
	...
	if (_score >= BIS_EVO_rank6 and _rank != "COLONEL")  exitWith  
	{
		_rname = format["Col.%1",_name];
		_hint =  format[_rankmes,_rname];
		["col","COLONEL",_hint] call BIS_EVO_Rdisp;		
