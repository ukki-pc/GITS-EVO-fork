...
while{LDL_dlg_display == "UAV"} do
{
	...
	//_structured = composeText [str(_infos select 0),lineBreak, str(_infos select 1)];
	...
	//#include "custom\range.sqf"
        (_display displayctrl 205) ctrlSetText format ["Range: %1 (Grid: %2)",round(LDL_ac130_plane distance (screenToWorld [0.5,0.5])), mapGridPosition (screenToWorld [0.5,0.5])];
