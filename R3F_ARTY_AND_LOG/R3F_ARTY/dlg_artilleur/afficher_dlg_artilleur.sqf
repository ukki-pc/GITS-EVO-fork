/**
 * Ouvre la bo�te de dialogue de l'artilleur et affiche en continu l'orientation du canon (azimut et �l�vation)
 * 
...
R3F_ARTY_fil_exec_affichage_dlg_artilleur = [] spawn
{
	...
	if (vehicle player != player) then
	{
		...
		// R�cup�ration des control des champs de texte
		#include "dlg_constantes.h"
		
		R3F_ARTY_dlg_artilleur_ouverte = 0;
		
		// Tant qu'il ne descend pas de la pi�ce ou ne ferme pas l'interface
		while {vehicle player == _piece} do
		{
			// Si il est en tireur, on met � jour les param�tres de la pi�ce
			if (gunner _piece == player && !isNil "R3F_ARTY_dlg_artilleur_ouverte") then
			{
				...
				// Calcul de l'azimut du canon avec une pr�cision au centi�me
				...
				// Calcul de l'�levation du canon avec une pr�cision au centi�me
				_elevation = asin ((_piece weaponDirection _canon) select 2);
				_elevation = round (_elevation*100) / 100;
				
				// Formatage de l'azimut � 2 d�cimales
				...
				// Formatage de l'�l�vation � 2 d�cimales
				...
				// Affichage � l'�cran des param�tres de la pi�ce
