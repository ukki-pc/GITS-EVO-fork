/**
 * Execute une mission de tir re�u pour une IA locale � la machine actuelle
 * 
 * @param 0 le nom de l'�metteur de l'ordre
 * @param 1 l'IA devant �xecuter la mission de tir
 * @param 2 azimut du tir
 * @param 3 �l�vation du tir
...
if (_poursuivre) then
{
	...
	// Si l'acc�s � l'ex�cution de mission de tir est v�rouill�, on laisse le fil d'ex�cution en cours l'ex�cuter
	...
	{
		// On v�rouille l'acc�s � l'ex�cution de missions de tir pour l'IA
		_tireur setVariable ["R3F_ARTY_mission_tir_IA_tir_en_cours", true, false];
		
		// On lance le fil d'ex�cution qui va traiter toute la file d'attente
		[_tireur, _piece] spawn
		{
			...
			} do
			{
				...
				// La munition demand�e est-elle compatible avec la pi�ce ?
				_idx_type_piece = -1;
				...
				{
					{
						...
						{
							if ((_x select 0) == (_chargeur_demande select 0) &&
								(_x select 1) == (_chargeur_demande select 1) &&
								(_x select 2) == (_chargeur_demande select 2) &&
								(_x select 3) == (_chargeur_demande select 3) &&
								(_x select 4) == (_chargeur_demande select 4)) then
							{
				...
				if (_mun_compatible) then
				{
					...
					// Charger la bonne munition si besoin et si un PC est � proximit�
					_mun_ok = true;
					if (_piece ammo (weapons _piece select 0) == 0 ||
						(_chargeur_actuel select 0) != (_chargeur_demande select 0) ||
						(_chargeur_actuel select 1) != (_chargeur_demande select 1) ||
						(_chargeur_actuel select 2) != (_chargeur_demande select 2) ||
						(_chargeur_actuel select 3) != (_chargeur_demande select 3) ||
						(_chargeur_actuel select 4) != (_chargeur_demande select 4)) then
					{
					...
					if (_mun_ok) then
					{
						...
						// Si on change de zone de tir, on passe du temps � viser, sinon c'est rapide
						...
						// V�rification que l'IA est bien pr�te � tirer
						// TODO re-implement for v1.6
						_tir_possible = true;
						/*_tir_possible = false;
						for [{_nb_tests = 0}, {_nb_tests < 10 && !_tir_possible}, {_nb_tests = _nb_tests+1}] do
						{
							private ["_azimut_reel", "_elevation_reel"];
							
							_azimut_reel = ((_piece weaponDirection (weapons _piece select 0)) select 0) atan2 ((_piece weaponDirection (weapons _piece select 0)) select 1);
							_elevation_reel = asin ((_piece weaponDirection (weapons _piece select 0)) select 2);
							
							if (abs (_azimut_reel - _azimut) < 4 && abs (_elevation_reel - _elevation) < 4) then
							{
								_tir_possible = true;
							}
							else
					...
					if (local _tireur && gunner _piece == _tireur && alive _tireur && alive _piece) then
					{
						// Recharger si la pi�ce est � court du munition et qu'un un PC est � proximit�
				...
				if (local _tireur && gunner _piece == _tireur && alive _tireur && alive _piece) then
				{
					// On marque l'ordre comme ex�cut�
					_ordres_tirs = + (_tireur getVariable "R3F_ARTY_mission_tir_IA_ordres_recus");
			...
			if (local _tireur && gunner _piece == _tireur && alive _tireur && alive _piece) then
			{
				...
				// On d�v�rouille l'acc�s � l'ex�cution de missions de tir pour l'IA
